"""
This file contains a number of functions and the class OGen (CLIPNET Generator) that
...
class OGen(tf.keras.utils.Sequence):
    def __init__(
        self,
        seq_folds,
        procap_folds,
    ...
    def __getitem__(self, index):
        ...
        y_pl = np.sum(y_batch[:, : y_batch.shape[1] // 2], axis=-1) + self.pseudocount
        y_mn = np.sum(y_batch[:, y_batch.shape[1] // 2 :], axis=-1) + self.pseudocount
        y = np.max([y_pl, y_mn], axis=0) / (y_pl + y_mn)
        return X, y
